...
'''
Instead of Unflatten we can create a Reshape function
...
class AE(nn.Module):
    ...
    def __init__(self, sequence_len):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(int(sequence_len/2), sequence_len * 8),
            nn.ReLU(),
            nn.Dropout(0.2),
    ...
    def reparameterize(self, z_mu, z_log_var, n=1):
        ...
        noise_distribution = torch.distributions.LogNormal(0, 0.001)  # 0=mean of log of distribution,  0.001=standard deviation of log of the distribution
        #device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        eps = noise_distribution.sample(z_log_var.size())  #.to(device)         # sample() method returns a list with a randomly selection of a specified number of items from a sequnce.
...
class DAEDisaggregator(Disaggregator):
    ...
    def train_on_chunk(self, mainchunk, meterchunk, epochs, batch_size):
        ...
        for epoch in range(epochs):  # prepei na tautizetai me to epochs sto dae.train(...,epochs=__,...) ???????
            for x, y in dataloader:
                ...
                encoded, z_mean, z_log_var, decoded = self.model(x, s)
    ...
    def disaggregate_chunk(self, mains):
        ...
        encoded_pred, z_mean_pred, z_log_var_pred, decoded_pred= self.model(X_batch,s)
        #print(pred)
        #print(decoded_pred)
